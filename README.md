🚀 DevOps Portfolio

This repository showcases real-world DevOps projects focused on containerisation, cloud infrastructure, and production-style deployments.

📦 Projects

🔹 AWS: EC2 + NGINX + Custom Domain
Deployed a web server on AWS EC2 and connected a custom domain using Cloudflare with HTTPS.

Tech: AWS EC2, NGINX, Cloudflare
Highlights:

- Real-world server deployment
- DNS + HTTPS configuration
- Troubleshooting real-world networking issues

Impact:

- Demonstrates end-to-end deployment from DNS to web server response
- Shows ability to debug real-world issues (SSH permissions, timeouts, SSL)

👉 https://github.com/TheMudda6/devops-portfolio/tree/main/aws-ec2-nginx-deployment

🔹 Docker: Flask + Redis + NGINX (Load Balanced App)
A multi-container application using Docker Compose with load balancing via NGINX.

Tech: Docker, Flask, Redis, NGINX
Highlights:

- Multi-container architecture
- Redis-backed request counter
- NGINX reverse proxy with load balancing

👉 https://github.com/TheMudda6/Docker-Projects

🔹 Git Workflows & Fundamentals
- Structured repository documenting Git commands, workflows, and real examples.

Tech: Git, GitHub
Highlights:

- Branching strategies
- Rebase vs merge
- Practical examples

👉 https://github.com/TheMudda6/git-Workflows

🧠 Skills Demonstrated

- Docker & containerisation
- Multi-container orchestration (Docker Compose)
- Reverse proxy & load balancing (NGINX)
- Cloud infrastructure (AWS EC2)
- DNS & HTTPS configuration (Cloudflare)
- Version control (Git & GitHub)

🎯 What I’m Working On

- Learning Kubernetes
- Exploring CI/CD pipelines
- Expanding cloud deployments

👨‍💻 About Me

Aspiring DevOps Engineer building hands-on projects to develop real-world skills in cloud infrastructure, automation, and containerisation.
